...
def init_db():
    global autocomplete
    workingHashes = csv.reader(open('ExampleHashes.p','r'))
    workingPrices = csv.reader(open('ExamplePrices.p','r'))
    cardHashes=[]
    priceList=[]
...
try:
    ...
    gaussianKernel = int(config[4][1])
    gaussianSubtract = int(config[5][1])
...
class infoDialog(QtGui.QDialog):
    ...
    def setup_infoGUI(self):
        self.setWindowIcon(QtGui.QIcon(os.path.join('Database','Misc','Icon.png')))
...
class MainApp(QtGui.QWidget):
    ...
    def display_video_stream(self):
        ...
        if findCard==True:      
            ...
            gray = cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,gaussianKernel,gaussianSubtract)
        ...
        if foundCard==True:
            ...
            cv2.polylines(frame,[pts],True,(50,255,50),2,lineType=cv2.LINE_AA)
            ...
            for sets in range(len(setMatches)):
                icon = cv2.imread(os.path.join('Database','SetArt',setMatches[sets][1]+'.png'))
                if sets==currentPick:
